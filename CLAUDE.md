...
The design system (tokens + Button/Badge/Panel/Toast/AppShell/Sidebar/ThemeToggle) **is** wired up — see [Design system](#design-system) below. Additional primitives (input, dialog, dropdown, etc.) are added on demand, per feature, not speculatively.
...
**Color tokens** (oklch, light/dark pair via `.dark` class, named after Apple's semantic color roles): `bg`, `bg-secondary`, `bg-elevated`, `border`, `border-subtle`, `label` (primary text), `secondary-label`, `tertiary-label`, `accent`, `accent-subtle`, `danger` (form/validation errors only — destructive _actions_ use undo-toast, not red, per below). Used as Tailwind utilities directly, e.g. `bg-bg-secondary`, `text-label`, `border-border-subtle`.
...
**Client-only persisted state (localStorage, etc.) has the same hydration issue as dark mode** — the value is genuinely unknown during SSR. Same fix: read it in a `useEffect`, and silence `react-hooks/set-state-in-effect` on that line with a one-line comment explaining why (see `src/features/crop/hooks/use-aspect-ratios.ts` for a working example). Don't work around this by moving the read into a lazy `useState` initializer — that still diverges between server and client and just trades a lint error for a silent hydration mismatch.
